...
class PacienteTest {
    ...
    void testAddyTodosTratamiento() {
        ...
        for (int i = 0; i < 1000; i++) {
            p.nuevoTratamiento(cod+i, descr, fecha, precio);
        }

        assertEquals(1000, p.todosTratamientos().length); 
	...
	void testSetValor() {
		...
		Paciente p = new Paciente(nombre, apellidos, sTelefono, sEmail, sDni, sFNac);
		p.setsNombre("Germán");
		assertEquals("Germán", p.getsNombre());
	...
	void testToString() {
		Paciente p = new Paciente(nombre, apellidos, sTelefono, sEmail, sDni, sFNac);
		assertEquals(String.format("%15s#%25s#%9s#%10s#%20s#%13s", nombre, apellidos, sDni, sFNac, sEmail,sTelefono),p.toString());
	}
